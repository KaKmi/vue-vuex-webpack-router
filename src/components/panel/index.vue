<template>
  <div class="weui_panel weui_panel_access">
    <div class="weui_panel_hd">今天-2015年12月18日星期一</div>
    <div class="weui_panel_bd">
      <a href="javascript:void(0);" class="weui_media_box weui_media_appmsg">
        <div class="weui_media_hd">
          <img class="weui_media_appmsg_thumb" src="data:image/png;base64," alt="">
        </div>
        <div class="weui_media_bd">
          <h4 class="weui_media_title">体育频道-手机搜狐</h4>
          <p class="weui_media_desc">m.sohu.com</p>
        </div>

       <span class="more-control" @click="showActionsheet"><i class="fa fa-ellipsis-h more-control-btn"></i></span>

      </a>

      <a href="javascript:void(0);" class="weui_media_box weui_media_appmsg">
        <div class="weui_media_hd">
          <img class="weui_media_appmsg_thumb" src="data:image/png;base64," alt="">
        </div>
        <div class="weui_media_bd">
          <h4 class="weui_media_title">体育频道-手机搜狐</h4>
          <p class="weui_media_desc">m.sohu.com</p>
        </div>

        <span class="more-control"><i class="fa fa-ellipsis-h more-control-btn"></i></span>

      </a>
    </div>
  </div>
  <div class="weui_panel weui_panel_access">
    <div class="weui_panel_hd">今天-2015年12月18日星期一</div>
    <div class="weui_panel_bd">
      <a href="javascript:void(0);" class="weui_media_box weui_media_appmsg">
        <div class="weui_media_hd">
          <img class="weui_media_appmsg_thumb" src="data:image/png;base64," alt="">
        </div>
        <div class="weui_media_bd">
          <h4 class="weui_media_title">体育频道-手机搜狐</h4>
          <p class="weui_media_desc">m.sohu.com</p>
        </div>

        <span class="more-control"><i class="fa fa-ellipsis-h more-control-btn"></i></span>

      </a>

      <a href="javascript:void(0);" class="weui_media_box weui_media_appmsg">
        <div class="weui_media_hd">
          <img class="weui_media_appmsg_thumb" src="data:image/png;base64," alt="">
        </div>
        <div class="weui_media_bd">
          <h4 class="weui_media_title">体育频道-手机搜狐</h4>
          <p class="weui_media_desc">m.sohu.com</p>
        </div>

        <span class="more-control"><i class="fa fa-ellipsis-h more-control-btn"></i></span>

      </a>
    </div>
  </div>
  <div class="weui_panel weui_panel_access">
    <div class="weui_panel_hd">今天-2015年12月18日星期一</div>
    <div class="weui_panel_bd">
      <a href="javascript:void(0);" class="weui_media_box weui_media_appmsg">
        <div class="weui_media_hd">
          <img class="weui_media_appmsg_thumb" src="data:image/png;base64," alt="">
        </div>
        <div class="weui_media_bd">
          <h4 class="weui_media_title">体育频道-手机搜狐</h4>
          <p class="weui_media_desc">m.sohu.com</p>
        </div>

        <span class="more-control"><i class="fa fa-ellipsis-h more-control-btn"></i></span>

      </a>

      <a href="javascript:void(0);" class="weui_media_box weui_media_appmsg">
        <div class="weui_media_hd">
          <img class="weui_media_appmsg_thumb" src="data:image/png;base64," alt="">
        </div>
        <div class="weui_media_bd">
          <h4 class="weui_media_title">体育频道-手机搜狐</h4>
          <p class="weui_media_desc">m.sohu.com</p>
        </div>

        <span class="more-control"><i class="fa fa-ellipsis-h more-control-btn"></i></span>

      </a>
    </div>
  </div>
  <actionsheet :show.sync="actionsheetShow" :menus="actionSheetMenus" :actions="actionSheetActions"
               @weui-menu-click="handleMenuClick" @weui-action-click="handleActionClick"></actionsheet>



</template>

<script>
  import {Actionsheet} from 'vue-weui';
 export default{
   data() {
     return {
       actionsheetShow: false,
       actionSheetMenus: {
         menu1: '解除屏蔽',
         menu2: '删除记录'

       },
       actionSheetActions: {
         action1: '取消'
       }
     }
   },
   components: {

     Actionsheet,
   },
   methods:{
     showActionsheet(){
       this.actionsheetShow=true
     },
     handleMenuClick(key){
       alert(`你点击了菜单【${key}】`);
     },
     handleActionClick(key){
       alert('你点击了操作项，Actionsheet要关闭了');
     }
   }
 }

</script>
<style>
  .weui_panel{
    margin-top: 0;
  }
  .weui_panel_hd{
    background-color: #f6f6f6;
    margin-top: 0;
  }
  .weui_panel_hd:after{
    display: none;
  }
  .weui_media_box .weui_media_title{
    max-width: 200px;

  }
  .more-control{
    width: 30px;
    height: 30px;
    border-radius: 50%;
    background: #ddd;
    text-align: center;
  }
  .more-control .more-control-btn{
    margin-top: 8px;
    color: #fff;
  }
</style>
